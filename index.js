...
/* ===============================================================
filter js for index.html
==================================================================*/

/* Shared filtering and page-specific rendering */
...
function renderLostFound() {
  ...
  if (!lostForm || !foundForm) return;

  function getLostPets() {
    return JSON.parse(localStorage.getItem("adoptme_lost_pets")) || [];
  }

  function getFoundPets() {
    return JSON.parse(localStorage.getItem("adoptme_found_pets")) || [];
  }
  ...
  function findMatches() {
    ...
    lostPets.forEach(lost => {
      foundPets.forEach(found => {
        ...
        const locationMatch = lost.location.toLowerCase().includes(found.location.toLowerCase()) ||
                              found.location.toLowerCase().includes(lost.location.toLowerCase());

        if (speciesMatch && colorMatch && locationMatch) {
  ...
  function renderData() {
    ...
    lostPetsList.innerHTML = lostPets.length
      ? lostPets.map(pet => `
        <div class="list-item">
          <strong>${pet.name}</strong><br>
          <small>${pet.species} • ${pet.color} • ${pet.location}</small>
        </div>
      `).join("")
      : `<div class="list-item">No lost pets reported yet.</div>`;

    matchesList.innerHTML = matches.length
      ? matches.map(match => `
        <div class="list-item">
          <strong>Possible match for ${match.lost.name}</strong><br>
          <small>${match.found.species} • ${match.found.color} • ${match.found.location}</small>
        </div>
      `).join("")
      : `<div class="list-item">No matches found yet.</div>`;
  }
  ...
  lostForm.addEventListener("submit", (e) => {
    ...
    localStorage.setItem("adoptme_lost_pets", JSON.stringify(lostPets));
    lostForm.reset();
    renderData();
  });
  ...
  foundForm.addEventListener("submit", (e) => {
    ...
    localStorage.setItem("adoptme_found_pets", JSON.stringify(foundPets));
    foundForm.reset();
    renderData();
  });
...
function renderEvents() {
  ...
  if (!eventsGrid || !eventSelect || !eventForm) return;
  ...
  eventForm.addEventListener("submit", (e) => {
    ...
    const registrations = JSON.parse(localStorage.getItem("adoptme_event_registrations")) || [];
    registrations.push({
      id: Date.now(),
      name: document.getElementById("eventName").value.trim(),
      email: document.getElementById("eventEmail").value.trim().toLowerCase(),
      eventTitle: document.getElementById("eventSelect").value,
      createdAt: new Date().toLocaleString()
    });

    localStorage.setItem("adoptme_event_registrations", JSON.stringify(registrations));
    showMessage("eventMessage", "You have been registered for the event!", "success");
    eventForm.reset();
  });
}