// ============ ADD THIS AT THE VERY TOP (MongoDB Connection for Events) ============
...
async function loadEventsFromMongoDB() {
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            // Format events to match your existing structure
            ...
            // Save to localStorage so existing code can read it
...
// Load event registrations from MongoDB
async function loadEventRegistrationsFromMongoDB() {
    try {
        const response = await fetch(`${API_BASE_URL}/events/registrations/all`);
        const result = await response.json();
        
        if (result.success && result.data && result.data.length > 0) {
            // Format registrations to match your existing structure
            ...
            console.log(`✅ Loaded ${formattedRegistrations.length} event registrations from MongoDB`);
...
async function loadAlertsFromMongoDB() {
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            // Format alerts to match your existing structure
...
// Save event registration to MongoDB
async function saveEventRegistrationToMongoDB(registrationData) {
    try {
        ...
        const result = await response.json();
        if (result.success) {
            console.log('✅ Event registration saved to MongoDB');
            return result.data;
        }
        return null;
    } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await loadEventRegistrationsFromMongoDB();
  await loadAlertsFromMongoDB();  // NEW: Load alerts from MongoDB
  
  const page = document.body.dataset.page;
  if (page !== "events") return;

  const eventsGrid = document.getElementById("eventsGrid");
  const eventAlertsList = document.getElementById("eventAlertsList");
  const eventForm = document.getElementById("eventForm");
  const eventRegistrationsList = document.getElementById("eventRegistrationsList");

  const eventSelect = document.getElementById("eventSelect");
  const eventEmailError = document.getElementById("eventEmailError");
  const eventPhoneError = document.getElementById("eventPhoneError");
  const eventConsentError = document.getElementById("eventConsentError");

  const defaultEvents = [
    {
      id: "EVT-1001",
      title: "Adoption Awareness Drive",
      date: "2026-05-12",
      city: "Kalyan",
      venue: "Pet City Community Hall",
      category: "Adoption",
      seats: 120,
      status: "Upcoming",
      description: "Meet rescued pets, interact with welfare volunteers, and learn about responsible pet adoption."
    },
    {
      id: "EVT-1002",
      title: "Rescue Response Training Camp",
      date: "2026-05-20",
      city: "Thane",
      venue: "Animal Welfare Center",
      category: "Rescue",
      seats: 80,
      status: "Filling Fast",
      description: "Hands-on learning session for emergency reporting, rescue support, and animal first-response basics."
    },
    {
      id: "EVT-1003",
      ...
      date: "2026-06-02",
      city: "Mumbai",
      venue: "AdoptMe Support Office",
      category: "Volunteer",
      seats: 60,
      status: "Upcoming",
      description: "A verified volunteer networking and planning meetup for ongoing welfare initiatives and drives."
    }
  ];

  const defaultAlerts = [
    {
      title: "New Adoption Awareness Drive Announced",
      message: "Registration is now open for the Kalyan Adoption Awareness Drive. Limited seats available.",
      level: "upcoming"
    },
    {
      title: "Rescue Training Camp Filling Fast",
      message: "Seats are filling quickly for the Rescue Response Training Camp. Register early.",
      level: "filling"
    },
    {
      title: "Volunteer Meetup Registration Open",
      message: "Volunteer community meetup registration has been activated for all verified participants.",
      level: "upcoming"
    }
  ];
  ...
  function getEvents() {
    const stored = JSON.parse(localStorage.getItem("adoptme_events"));
    if (stored && stored.length) return stored;
    localStorage.setItem("adoptme_events", JSON.stringify(defaultEvents));
    return defaultEvents;
  }

  function getAlerts() {
    const stored = JSON.parse(localStorage.getItem("adoptme_event_alerts"));
    if (stored && stored.length) return stored;
    localStorage.setItem("adoptme_event_alerts", JSON.stringify(defaultAlerts));
  ...
  function clearErrors() {
    ...
    eventPhoneError.textContent = "";
    eventConsentError.textContent = "";
  }
  ...
  function renderEvents() {
    ...
    eventsGrid.innerHTML = events.map(event => `
      <div class="card event-card-modern">
        ...
        <small><strong>Venue:</strong> ${event.venue}</small><br>
        <small><strong>Seats:</strong> ${event.seats}</small><br>
        <span class="event-status-badge ${event.status === "Filling Fast" ? "filling" : event.status === "Closed" ? "closed" : "upcoming"}">
          ${event.status}
        </span>
      </div>
    `).join("");
  ...
  function renderAlerts() {
    ...
    if (!eventAlertsList) return;

    eventAlertsList.innerHTML = alerts.length
      ? alerts.map(alert => `
        <div class="list-item event-alert-item">
  ...
  function fillEventSelect() {
    ...
    eventSelect.innerHTML = `
      ...
      ${events.map(event => `<option value="${event.id}">${event.title} – ${event.city} (${event.date})</option>`).join("")}
    `;
  }
  ...
  function renderRegistrations() {
    ...
    if (!eventRegistrationsList) return;

    eventRegistrationsList.innerHTML = registrations.length
      ? registrations.slice().reverse().map(item => `
        <div class="list-item">
          <strong>${item.eventTitle}</strong><br>
          <small>${item.name} • ${item.city}</small><br>
          <small>${item.role} • ${item.createdAt}</small>
        </div>
      `).join("")
      : `<div class="list-item">No event registrations yet.</div>`;
  }
  ...
  if (eventForm) {
    eventForm.addEventListener("submit", async (e) => {
      ...
      const consentAccepted = document.getElementById("eventConsent").checked;

      const registration = {
        id: Date.now(),
        registrationId: "EVREG-" + Date.now(),
        userEmail: document.getElementById("eventEmail").value.trim().toLowerCase(),
        name: document.getElementById("eventName").value.trim(),
        email: document.getElementById("eventEmail").value.trim().toLowerCase(),
      ...
      if (!validatePhone(registration.phone)) {
        eventPhoneError.textContent = "Please enter a valid 10-digit Indian mobile number.";
        hasError = true;
      }

      if (!consentAccepted) {
        eventConsentError.textContent = "Please accept the event participation consent.";
        hasError = true;
      }

      if (!registration.eventId || !selectedEvent) {
        showMessage("eventMessage", "Please select a valid event.", "error");
        hasError = true;
      }
      ...
      const backendResult = await saveEventRegistrationToMongoDB(registration);
      if (backendResult) {
        console.log("✅ Event registration also saved to MongoDB Compass!");
      } else {
        console.warn("⚠️ Registration saved to localStorage only, backend may not be running");
      }

      showMessage(
        "eventMessage",
        `Event registration successful! You are registered for ${registration.eventTitle}.`,
        "success"
      );
